...
const { PORT, HOST, DEFAULT_MODEL, MEMORY_CAPTURE_ENABLED, MAX_MESSAGE_LENGTH, MAX_REQUEST_MESSAGES } = require("./lib/config");
...
db.exec(`CREATE TABLE IF NOT EXISTS memories (id INTEGER PRIMARY KEY AUTOINCREMENT, content TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
db.exec(`CREATE TABLE IF NOT EXISTS nexus_conversations (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL DEFAULT 'Nueva conversación', created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
db.exec(`CREATE TABLE IF NOT EXISTS nexus_messages (id INTEGER PRIMARY KEY AUTOINCREMENT, conversation_id INTEGER NOT NULL, role TEXT NOT NULL, content TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (conversation_id) REFERENCES nexus_conversations(id) ON DELETE CASCADE)`);
...
const nexus = createNexusService({
    ...
    webSearch,
    calculate,
    captureMemory: MEMORY_CAPTURE_ENABLED ? async (text) => {
        try {
...
app.use("/api", createChatRouter({ nexus, createConversation: conversations.createConversation, getConversation: conversations.getConversation, maxMessageLength: MAX_MESSAGE_LENGTH, maxRequestMessages: MAX_REQUEST_MESSAGES }));
app.use("/api", createHealthRouter({ memoryEnabled: MEMORY_CAPTURE_ENABLED }));
