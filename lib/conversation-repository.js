...
function createConversationRepository(db) {
    ...
    const saveMessage = (conversationId, role, content) => {
        db.prepare("INSERT INTO nexus_messages (conversation_id, role, content) VALUES (?, ?, ?)")
            .run(conversationId, role, content);
        db.prepare("UPDATE nexus_conversations SET updated_at = CURRENT_TIMESTAMP WHERE id = ?")
            .run(conversationId);
    };

    const updateConversationTitle = (conversationId, title) => {
        db.prepare("UPDATE nexus_conversations SET title = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
    ...
    return { createConversation, getConversations, getConversation, getMessages, saveMessage, updateConversationTitle, deleteConversation, clearMessages, searchConversations };
}
