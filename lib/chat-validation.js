...
function validateChatRequest(messages) {
    ...
    if (lastMessage?.role !== "user" || !userText) {
        return { ok: false, status: 400, error: "El último mensaje debe ser del usuario y no estar vacío." };
    }
    if (userText.length > MAX_MESSAGE_LENGTH) {
        return { ok: false, status: 413, error: `El mensaje supera el límite de ${MAX_MESSAGE_LENGTH} caracteres.` };
    }
