...
async function webSearch(query, { fetchImpl = fetch, endpoint = DEFAULT_ENDPOINT, timeoutMs = 15_000 } = {}) {
    ...
    try {
        ...
        if (error.name === "AbortError") throw new Error(`Timeout del buscador WEB (${timeoutMs}ms).`);
        throw error;
    } finally {
